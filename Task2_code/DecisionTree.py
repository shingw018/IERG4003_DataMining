...
def readtrain():
    ...
    for i in range(len(csv['Balance'])):
        if((csv['Balance'][i] == 0) and (csv['Tenure'][i] > 8)):
            csv['Balance'][i] = csv['Balance'].mean() + csv['Balance'].std()*1
...
def treeclass(trainset, testset, response, id):
    D = tree.DecisionTreeClassifier(min_samples_split=257, random_state=0)

    # GridSearchCV
    '''param_dist = {'min_samples_split':range(1,500,1), 'random_state':[0]}
    grid = GridSearchCV(D,param_dist,cv = 3,scoring = 'neg_log_loss')
    grid.fit(trainset,response)
    print(grid.best_params_)
    print(grid.best_estimator_)'''

    D.fit(trainset, response)

    pred = D.predict(testset)
    pred_response = []
    _id = []
    ...
    _format.to_csv('submission_2_DecisionTree.csv', index=False, header=True)
