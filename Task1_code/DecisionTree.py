...
def treeclass(trainset, testset, response, id):
    D = tree.DecisionTreeClassifier(criterion="gini", splitter='best', max_depth=None,
                                    min_samples_split=2, min_samples_leaf=1, max_features='sqrt', random_state=50)

    D.fit(trainset[['Gender', 'Age', 'Driving_License', 'Region_Code', 'Previously_Insured',
                    'Vehicle_Age', 'Vehicle_Damage', 'Annual_Premium', 'Policy_Sales_Channel', 'Vintage']], response)
    ...
    _format.to_csv('submission_1_DecisionTree.csv', index=False, header=True)
